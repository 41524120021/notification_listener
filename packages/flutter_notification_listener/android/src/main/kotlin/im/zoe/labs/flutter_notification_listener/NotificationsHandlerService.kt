...
class NotificationsHandlerService: MethodChannel.MethodCallHandler, NotificationListenerService() {
    ...
    override fun onMethodCall(@NonNull call: MethodCall, @NonNull result: MethodChannel.Result) {
      when (call.method) {
          ...
          "service.get_active_notifications" -> {
              ...
              Log.d(TAG, "get all active notifications from system")
              try {
                  // Use Android API to get active notifications
                  val activeNotifications = getActiveNotifications()
                  
                  if (activeNotifications == null) {
                      Log.w(TAG, "getActiveNotifications() returned null - service might not be bound properly")
                      return result.success(emptyList<Map<String, Any>>())
                  }
                  
                  Log.d(TAG, "Raw active notifications count: ${activeNotifications.size}")
                  
                  val notificationsList = activeNotifications.mapNotNull { sbn ->
                      try {
                          val evt = NotificationEvent(mContext, sbn)
                          val packageName = evt.data["package_name"]
                          val title = evt.data["title"]
                  ...
                  Log.d(TAG, "Successfully processed ${notificationsList.size} active notifications from system")
                  return result.success(notificationsList)
              } catch (e: Exception) {
                  Log.e(TAG, "error getting active notifications: ${e.message}")
                  e.printStackTrace()
                  return result.error("ERROR", "Failed to get active notifications", e.message)
              }
          }
    ...
    private fun cancelNotificationByUid(uid: String): Boolean {
        Log.d(TAG, "cancel notification by uid: $uid")
        if (!eventsCache.containsKey(uid)) {
            Log.d(TAG, "notification is not in cache: $uid")
            return false
        }
        val n = eventsCache[uid]
        if (n == null) {
